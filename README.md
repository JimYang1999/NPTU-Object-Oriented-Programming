...
## 物件導向基本觀念
物件導向觀念架構在「物件、類別、訊息、繼承、關聯」五種基本觀念之上。

- 物件：東西的靜態特性有顏色、長寬等等特性，也就是物件導向觀念內物件的**屬性(Attribute)**，而東西如果是活的，就會有動作，也就是東西的動態特性，在物件導向觀念內稱為物件的**方法(Method)**，例如狗叫就是狗這種生物的方法。
- 類別：類別為針對某一類物件的**特性描述**，也就是物件的抽象定義，比如：狗有四隻腳，狗的叫聲、名字...等。當要用這個程式模擬小黑這隻狗時，就要用**狗類別**產生一隻名叫**小黑**的**狗物件**，一個類別可以有好幾個物件，例如我們可以再用狗類別產生一隻名叫小黃的狗物件。<br>類別(class)包含了屬性與方法的定義，**屬性被稱為資料成員(data member)，方法被稱為成員函式(member function)**。
- 訊息：訊息在運用物件導向觀念模擬的電腦系統裡，相當於真實世界裡人說的話，藉由訊息的傳遞，觸動**物件**的方法，然後產生動作，完成系統功能。例如當你按下瀏覽器右上的XX，按下去(發送訊息給物件)後，瀏覽器會關閉(依據你按這個按鈕做回應，而這個回應動作就是物件的方法)。
- 繼承：軟體的**再利用**，被繼承的類別稱為**基礎類別(Base class) 或 父類別**，繼承基礎類別的類別稱為**衍生類別(Derived class) 或 子類別**。若繼承public，稱為**public derivation**，所有在父類別中的public成員都會變成是在子類別中的public成員。但在父類別中的private成員，則僅能從繼承自父類別中的public(或protected)成員函式來存取。除此之外，除了預設的建構函式外，其餘的建構函式並不會被繼承。
- 關聯：描述各物件間相互連接的結構關係。
